...
class MyHandler<K extends IVertexData> extends DefaultHandler {
  ...
  Map<String, PolygonMesh> meshes = new HashMap<>();
    ...
    switch (qName) {
      ...
      case "instance": {
       String path = "";
        for (int i = 0; i < attributes.getLength(); i++) {
          if (attributes.getQName(i).equals("name")) {
            Meshname = attributes.getValue(i);
      ...
      case "object": {
        ...
        if (objectname.length() > 0) {
          ...
          stackNodes.push(node);
          subgraph.put(stackNodes.peek().getName(), stackNodes.peek());
          stackNodes.peek().setMeshes(meshes);
        }
      }
      break;
  ...
  public void endElement(String uri, String localName, String qName) throws SAXException {
    ...
    switch (qName) {
      ...
      case "rotate":
        ...
        Vector3f axis = new Vector3f(sc.nextFloat(),sc.nextFloat(),sc
                .nextFloat());
        axis = axis.normalize();
        transform.rotate((float) Math.toRadians(angle), axis.x,axis
                .y,axis.z);
        break;
      case "translate":
        sc = new Scanner(data);
