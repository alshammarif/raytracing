...
public interface INode
{
    ...
    /**
     * Searches for lights in the scenegraph rooted at this node
     * @param modelView
     * @return lights and arraylist of lights in the scenegraph
     */
    ArrayList<util.Light> getLights(Stack<Matrix4f> modelView);

    /**
     * returns the min bounds of the current bounding box
     * @return the min bounds
     */
    Vector4f getMinBounds();

    /**
     * retruns the max bounds of the current bounding box
     * @return the max bounds
     */
    Vector4f getMaxBounds();

    /**
     * sets the bounding box for that node
     */
    void setBoundingBox();
     ...
     * @param meshes
     */
    void setMeshes(Map<String,PolygonMesh> meshes);

    public Vector4f getleftTopFront();

    public Vector4f getleftBotBack();

    public Vector4f getleftTopBack();

    public Vector4f getrightBotBack();

    public Vector4f getrightTopFront();

    public Vector4f getrightBotFront();

    public Vector4f getCenter();

    INode explodeNode(Vector4f pc);

    /**
     * adds the exploation transformation
     */
    IScenegraph expload(IScenegraph sg, IScenegraph blank);

    /**
     * casts a ray and returns the color at that point
     * @param r1
     * @param modelview
     */
